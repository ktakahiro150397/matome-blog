...
## 2025-05-13 記事読了時間表示機能の実装

### 実施内容

- Prismaスキーマに`readingTimeMinutes`フィールド（整数型）を追加。
- マイグレーション（`add_reading_time`）を作成・実行。
- `/src/lib/readingTime.ts`に読了時間計算ユーティリティ関数を実装。
  - 日本語の平均読書速度（500文字/分）に基づいて計算。
  - 空白や改行を除去して純粋な文字数をカウント。
  - 最低1分を返すように実装。
- 読了時間計算関数のユニットテスト（`/src/lib/readingTime.test.ts`）を実装。
- 同期スクリプト（`scripts/sync-content.ts`）を拡張し、MDXファイル読み込み時に読了時間を計算してDBに保存するよう修正。
- 同期スクリプトを実行し、既存の全記事の読了時間を計算・DB更新。

### 現状

- 全記事の`readingTimeMinutes`がDBに保存済み。
- 読了時間計算ユーティリティのテストがパス。
- 型・ESLintエラーが一部あるものの、今回の実装とは無関係。

### 次のアクション

- BlogCardコンポーネントに読了時間表示UIを追加。
- 記事詳細ページ(/blog/[slug]/page.tsx)に読了時間表示UIを追加。
- UI表示のテスト作成。
- 残りのLintエラー修正を検討。
...
## 2025-05-12 ヘッダー検索リンクの横並び修正

### 実施内容

- ヘッダーの検索リンク（/src/components/layout/header.tsx）でアイコンと「検索」テキストが縦並びになっていた問題を、flex-row指定で横並び・中央揃えに修正。
- `npm test` を実行し、全テストパスを確認。
- 追加・変更ファイルの型・ESLintエラーなしを確認。

### 現状

- 検索リンクのアイコンとテキストが横並びで正しく表示される。
- テスト・型・ESLintエラーなし。
- Memory Bankも最新化。

### 次のアクション

- ヘッダーの「カテゴリー」機能の具体的な要件が決まり次第、実装を検討。
...
matome-blogでは、タグに「グループ」属性（例: group, category）を追加することで、仮想的なカテゴリ分けを実現する方針とする。

- PrismaのTagモデルに `group: String?` カラムを追加し、タグごとに「技術」「エンタメ」「学習」などの大分類を持たせる。
- UIでは「グループごとにタグをまとめて表示」や「グループ単位でフィルタ」などカテゴリ的な見せ方が可能。
- タグ追加・編集時にグループを指定し、グループ未設定（null）のタグも許容。
- データ構造はタグのまま、UIのみカテゴリ風に見せる。
- 実装時はPrismaスキーマ・マイグレーション・UI設計を一括で対応する。

この方針により、タグの柔軟性を保ちつつカテゴリ的な大分類も実現できる。今後「カテゴリー」機能が必要になった場合はこの設計に従い実装する。
